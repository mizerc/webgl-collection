class Webgl_v1 {
  constructor(canvas) {
    ...
    Webgl_v1.printGpuSpecs(this.gl);
    this.bufferNames = {};
    this.attributeVars = {};
  }
  ...
  async loadVertexShader(id) {
    this.loadedVertexShader = await Webgl_v1.loadShaderFromScriptTag(id);
  }
  async loadFragmentShader(id) {
    this.loadedFragmentShader = await Webgl_v1.loadShaderFromScriptTag(id);
  }
  compiledVertexShader() {
    this.compiledVertexShader = Webgl_v1.compileShader(
      this.gl,
  ...
  compiledFragmentShader() {
    this.compiledFragmentShader = Webgl_v1.compileShader(
      this.gl,
  ...
  populateBuffer(bufferName, vertexData) {
    const gl = this.gl;
    gl.bufferData(gl.ARRAY_BUFFER, vertexData, gl.STATIC_DRAW);
  }
  enableBuffer(bufferName) {
    const gl = this.gl;
  ...
  configureHowToParseAttributeVariable(
    attributVarName,
    NUMBER_OF_FLOATS,
    OFFSET_BYTES,
    STRIDE_BYTES
